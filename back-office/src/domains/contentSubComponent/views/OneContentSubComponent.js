...
import { fetchContentSubComponet, updateContentSubComponet } from '../../../store/contentsubcomponet';
...
function OneContentSubComponent() {
  ...
  console.log("🚀 ~ file: OneContentSubComponent.js:12 ~ OneContentSubComponent ~ contentsubcomponetId:", contentsubcomponetId)
  const contentsubcomponet = useSelector((state) => state?.contentsubcomponet?.contentsubcomponet);
  const [inputs, setInputs] = useState([]);
  const [show,setShow]= useState(false)
  const [readOnly, setReadOnly] = useState(true);
  const [aux, setAux] = useState(null);
  console.log("🚀 ~ file: OneContentSubComponent.js:17 ~ OneContentSubComponent ~ aux:", aux)
  useEffect(() => {
  ...
  const onSubmit = (e) => {
    ...
    dispatch(updateContentSubComponet({ title, content,navigateTo, contentsubcomponetId })).then(
      (result) => {
        if (!result.error) {
    ...
    >
      <div className="d-flex  justify-content-between align-items-center px-3 flex-wrap headerProfile">
        ...
        {readOnly && (
          ...
          >
          Edit {contentsubcomponet?.title}<i class="fa-solid fa-play fa-fade px-2"></i>
          </button>
        )}
      </div>
