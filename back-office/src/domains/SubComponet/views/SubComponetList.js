...
import { fetchSubComponets, removeSubComponet } from '../../../store/subComponet';
import { showErrorToast, showSuccessToast } from '../../../utils/toast';

function SubComponetList({mainId}) {
    console.log("🚀 ~ file: SubComponetList.js:14 ~ SubComponetList ~ mainId:", mainId)
    const navigate = useNavigate()
  const dispatch = useDispatch();
  const subcomponets = useSelector((state)=>state.subComponet.subcomponets.items)
  console.log("🚀 ~ file: SubComponetList.js:17 ~ SubComponetList ~ subcomponets:", subcomponets)
  ...
  />
  <Table columns={columns} rows={subcomponets?.length ? subcomponets : []} />
  {isOpen && (
    <DeleteModal
